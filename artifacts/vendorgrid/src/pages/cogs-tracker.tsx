...
import { Crown, TrendingUp, TrendingDown, DollarSign, Package, Plus, Trash2, ChevronDown, ChevronUp, BarChart3, ShoppingBag, Info, Boxes } from "lucide-react";
...
const STORAGE_KEY = "cogs-tracker-selection";
const INVENTORY_VALUE = "__inventory__";
...
function AddCogsRow({ catalogItemId, existingCategories, onSaved }: {
  catalogItemId: number;
  ...
  const availableCategories = DIRECT_CATEGORIES.filter(c => !existingCategories.includes(c));
  if (availableCategories.length === 0) return null;

  const addMutation = useMutation({
    mutationFn: ({ category, amountCents }: { category: string; amountCents: number }) =>
      apiRequest("PUT", `/api/vendor/cogs/item/${catalogItemId}`, { category, amountCents }),
    onSuccess: () => {
      setPendingCategory("");
      setPendingCost("");
  ...
  return (
    <div className="flex items-center gap-2 pt-1">
      <Select value={pendingCategory} onValueChange={setPendingCategory}>
        <SelectTrigger className="rounded-xl text-sm flex-1" data-testid={`select-cogs-category-${catalogItemId}`}>
        ...
        <>
          <div className="relative w-32">
            ...
            <Input
              ...
              placeholder="0.00"
              data-testid={`input-cogs-cost-${catalogItemId}`}
            />
          </div>
          <Button
            size="sm"
            className="rounded-xl"
            disabled={!pendingCost || addMutation.isPending}
            onClick={() => addMutation.mutate({ category: pendingCategory, amountCents: parseDollars(pendingCost) })}
            data-testid={`button-add-cogs-${catalogItemId}`}
          >
          ...
          <CardContent className="p-5 space-y-5">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <AddCogsRow catalogItemId={item.catalogItemId} existingCategories={existingCategories} onSaved={onSaved} />
              </div>
            </div>
...
function InventoryItemCard({ item, onSaved }: { item: InventoryItem; onSaved: () => void }) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  ...
  const sellPriceCents = parseDollars(sellPrice);
  const directCogs = item.cogs.reduce((s, c) => s + c.amountCents, 0);
  const margin = sellPriceCents - directCogs;
  const isAssigned = item.assignments.length > 0;

  return (
    <Card className="rounded-2xl overflow-hidden" data-testid={`card-inv-item-${item.catalogItemId}`}>
      <button
        className="w-full px-5 py-4 flex items-center gap-3 hover:bg-muted/30 transition-colors text-left"
        onClick={() => setExpanded(e => !e)}
          ...
          <div className="flex items-center gap-2 flex-wrap mt-0.5">
            {isAssigned ? (
              <span className="text-xs text-muted-foreground flex items-center gap-1">
                ...
                {item.totalAssigned} assigned across {item.assignments.length} event{item.assignments.length !== 1 ? "s" : ""}
              </span>
            ) : (
              <span className="text-xs text-muted-foreground italic">Not assigned to any event</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-4 shrink-0">
          <div className="text-right hidden sm:block">
            <p className="text-xs text-muted-foreground">Margin/Unit</p>
            <ProfitBadge cents={margin} />
          </div>
          {expanded ? <ChevronUp className="w-4 h-4 text-muted-foreground" /> : <ChevronDown className="w-4 h-4 text-muted-foreground" />}
        </div>
      </button>
      ...
      {expanded && (
        <div className="border-t border-border/50">
          <CardContent className="p-5 space-y-5">
            ...
            {isAssigned && (
              <div className="rounded-xl bg-blue-50/50 dark:bg-blue-900/10 border border-blue-200/50 dark:border-blue-700/30 p-3">
                <div className="flex items-start gap-2">
                  <Info className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-blue-700 dark:text-blue-400 mb-1">Event Assignments</p>
                    <div className="space-y-1">
                      {item.assignments.map(a => (
                        <div key={a.eventId} className="flex items-center justify-between gap-2">
                          <span className="text-xs text-blue-700 dark:text-blue-300 truncate">{a.eventTitle}</span>
                          <Badge variant="outline" className="text-xs border-blue-300 text-blue-600 dark:border-blue-600 dark:text-blue-400 shrink-0">
                            {a.quantityAssigned} assigned
                          </Badge>
                        </div>
                      ))}
                    </div>
              ...
              <div className="flex items-center gap-2 max-w-[200px]">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    className="pl-6 rounded-xl text-sm"
                    placeholder="0.00"
                    data-testid={`input-inv-price-${item.catalogItemId}`}
                  />
                </div>
                {sellPriceDirty && (
                  <Button
                    ...
                    disabled={updateSellPriceMutation.isPending}
                    onClick={() => updateSellPriceMutation.mutate(parseDollars(sellPrice))}
                    data-testid={`button-save-inv-price-${item.catalogItemId}`}
                  >
                    Save
                  </Button>
                )}
            ...
            <div>
              ...
              <div className="space-y-2">
                {item.cogs.length === 0 && (
                  <p className="text-xs text-muted-foreground italic px-1">No costs entered yet. Add one below.</p>
                )}
                {item.cogs.map(entry => (
                  <CogsEntryRow
                    key={entry.category}
            ...
            {/* Summary — margin only (no event overhead in inventory view) */}
            <div className="grid grid-cols-3 gap-3">
              <SummaryCell label="Direct COGS/Unit" value={fmt(directCogs)} neutral />
              <SummaryCell label="Sell Price" value={fmt(sellPriceCents)} neutral />
              <SummaryCell label="Margin/Unit" value={fmt(margin)} profit={margin} highlight />
            </div>
            <p className="text-xs text-muted-foreground text-center -mt-2">
              Event overhead (booth, travel, lodging) is applied per-event in the event view.
            </p>
          </CardContent>
        </div>
      )}
...
export default function CogsTrackerPage() {
  ...
  const isInventoryMode = selection === INVENTORY_VALUE;
  const selectedEventId = (!isInventoryMode && selection) ? parseInt(selection) : null;
  ...
  const { data: inventorySummary, isLoading: inventoryLoading } = useQuery<InventorySummary>({
    ...
    enabled: !!hasAccess && isInventoryMode,
  });

  const refreshSummary = useCallback(() => {
    if (isInventoryMode) {
      queryClient.invalidateQueries({ queryKey: ["/api/vendor/cogs/inventory"] });
    } else {
      queryClient.invalidateQueries({ queryKey: ["/api/vendor/cogs/summary", selectedEventId] });
    }
  }, [queryClient, selectedEventId, isInventoryMode]);
  ...
  const invTotalMargin = inventorySummary?.items.reduce((s, i) => s + i.marginCents, 0) ?? 0;
  const invTotalCogs = inventorySummary?.items.reduce((s, i) => s + i.directCogsCents, 0) ?? 0;
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Track your cost of goods and profitability per item.</p>
        </div>
      ...
      <Card className="rounded-2xl">
        <CardContent className="p-5">
          <label className="text-sm font-semibold block mb-2">Select View</label>
          <Select
            value={selection}
            onValueChange={setSelection}
          >
            <SelectTrigger className="rounded-xl max-w-sm" data-testid="select-cogs-view">
              <SelectValue placeholder="Choose an event or view…" />
            </SelectTrigger>
            <SelectContent>
          ...
          {eventOptions.length === 0 && !isInventoryMode && (
            <p className="text-xs text-muted-foreground mt-2">
              No events with assigned items yet. Select "All Inventory" to manage COGS for all catalog items.
            </p>
          )}
        </CardContent>
      </Card>
        ...
        inventoryLoading ? (
          ...
          <Card className="rounded-2xl">
            <CardContent className="flex flex-col items-center justify-center py-16 text-center gap-3">
              ...
              <div>
                <p className="font-semibold text-foreground">No catalog items yet</p>
                <p className="text-sm text-muted-foreground mt-1">Add items to your catalog to start tracking costs.</p>
              </div>
          ...
          <div className="space-y-4">
            {/* Inventory totals */}
            <div className="grid grid-cols-3 gap-3">
              <SummaryCell label="Items in Catalog" value={inventorySummary.items.length.toString()} neutral />
              <SummaryCell label="Avg Direct COGS/Item" value={fmt(Math.round(invTotalCogs / inventorySummary.items.length))} neutral />
              <SummaryCell label="Avg Margin/Item" value={fmt(Math.round(invTotalMargin / inventorySummary.items.length))} profit={invTotalMargin} highlight />
            </div>

            <div className="space-y-3">
              <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground px-1">
                All Catalog Items ({inventorySummary.items.length})
              </h2>
      ...
      {/* ── EVENT MODE ── */}
      {!isInventoryMode && selectedEventId && (
        <>
              ...
              <CardContent className="flex flex-col items-center justify-center py-16 text-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Go to your profile to assign catalog items to this event.
                  </p>
                </div>
                <Link href="/profile?tab=catalog">
                  <Button variant="outline" size="sm" className="rounded-xl mt-1">Manage Catalog</Button>
      ...
      {!selection && (
        <Card className="rounded-2xl">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center gap-3">
            ...
            <div>
              <p className="font-semibold text-foreground">Choose a view to get started</p>
              <p className="text-sm text-muted-foreground mt-1">
                Select "All Inventory" to manage costs across all items, or pick a specific event to track event-level profitability.
              </p>
            </div>
          </CardContent>
        </Card>
