...
import { useVendorPosts } from "@/hooks/use-vendor-posts";
import { Link } from "wouter";
import { Calendar, MapPin, ArrowRight, Loader2, Sparkles, Package, Users, Image as ImageIcon } from "lucide-react";
...
export default function Home() {
  ...
  // Fetch all posts for all events to show vendor counts and item gallery
      ...
      <Tabs defaultValue="markets" className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <TabsList className="bg-muted/50 p-1 rounded-xl h-12 w-fit">
            <TabsTrigger value="markets" className="rounded-lg px-6 h-10 data-[state=active]:bg-background data-[state=active]:shadow-sm gap-2">
              <Calendar className="w-4 h-4" />
              Markets
            </TabsTrigger>
            <TabsTrigger value="items" className="rounded-lg px-6 h-10 data-[state=active]:bg-background data-[state=active]:shadow-sm gap-2">
          ...
          {isLoadingEvents || isLoadingPosts ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {events?.map((event, i) => {
                ...
                const uniqueVendors = Array.from(new Set(eventPosts.map(p => p.vendorId)));
                      ...
                      <div className="h-48 bg-muted relative overflow-hidden">
                        ...
                        {uniqueVendors.length > 0 && (
                          <div className="absolute bottom-4 left-4 flex -space-x-2">
                            {eventPosts.slice(0, 3).map((post, idx) => (
                      ...
                      <div className="p-6 flex-1 flex flex-col">
                        ...
                        <div className="space-y-2 mb-4 flex-1">
                          ...
                          <div className="flex items-center gap-2 text-muted-foreground text-sm">
                            ...
                            <span>{uniqueVendors.length} Vendors Registered</span>
                          </div>
                        </div>
                        
                        <div className="pt-4 border-t border-border/50 flex items-center justify-between mt-auto">
                          <span className="text-sm font-medium text-muted-foreground">
                            {eventPosts.length} items listed
                          </span>
        ...
        <TabsContent value="items" className="mt-0">
          {isLoadingPosts ? (
            ...
            <div className="text-center py-20 bg-card rounded-2xl border border-dashed border-border">
              ...
              <p className="text-muted-foreground">No items have been posted by vendors yet.</p>
            </div>
          ) : (
              ...
              {allPosts.map((post) => (
                <Link href={`/events/${post.eventId}`} key={post.id}>
                  <div className="group bg-card rounded-2xl overflow-hidden border border-border/50 shadow-sm hover:shadow-md transition-all cursor-pointer">
                    <div className="aspect-square bg-muted relative">
                      {post.imageUrl ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center text-muted-foreground/40">
                          ...
                          <span className="text-xs uppercase tracking-tighter">No Photo</span>
                        </div>
                      )}
                    ...
                    <div className="p-4">
                      <p className="text-sm text-foreground font-medium line-clamp-2 min-h-[2.5rem]">
                      ...
                      <div className="mt-3 flex items-center justify-between text-[10px] uppercase tracking-wider font-bold text-muted-foreground">
                        <span>{events?.find(e => e.id === post.eventId)?.title || "Event"}</span>
                      </div>
                    </div>
                  </div>
                </Link>
